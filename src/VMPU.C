...
/* CARD_AUDIGY: MIDI can render on the EMU10K2's own hardware voices
 * (emu_wt.c) instead of TinySoundFont. One synth per boot: the hardware
 * wavetable wins if a soundfont was loaded into it, TSF handles /SF:.
 * The dispatchers keep the message parser below synth-agnostic; HWWT folds
 * to 0 on every other build, leaving the TSF path byte-identical. */
#ifdef CARD_AUDIGY
#include "emu_wt.h"
#define HWWT EMUWT_Active()
#else
#define HWWT 0
#endif

static void synth_note_on(int ch, int key, int vel)
{
    if (HWWT) { EMUWT_NoteOn(ch, key, vel); return; }
    tsf_channel_note_on(tsfrenderer, ch, key, vel / 127.0f);
}

static void synth_note_off(int ch, int key)
{
    if (HWWT) { EMUWT_NoteOff(ch, key); return; }
    tsf_channel_note_off(tsfrenderer, ch, key);
}

static void synth_program(int ch, int prog)
{
    if (HWWT) { EMUWT_ProgramChange(ch, prog); return; }
    tsf_channel_set_presetnumber(tsfrenderer, ch, prog, ch == 9);
}

static void synth_control(int ch, int cc, int val)
{
    if (HWWT) { EMUWT_ControlChange(ch, cc, val); return; }
    tsf_channel_midi_control(tsfrenderer, ch, cc, val);
}

static void synth_bend(int ch, int bend14)
{
    if (HWWT) { EMUWT_PitchBend(ch, bend14); return; }
    tsf_channel_set_pitchwheel(tsfrenderer, ch, bend14);
}

static void synth_pressure(int ch, int val)
{
    if (HWWT) { EMUWT_ChannelPressure(ch, val); return; }
    tsf_channel_set_pressure(tsfrenderer, ch, val / 127.f);
}

/* master volume, 0-127 (GM sysex / GS volume) */
static void synth_master(int v)
{
    if (v > 127) v = 127;
    if (HWWT) {
        EMUWT_SetMasterVolume(v >= 127 ? 32768u : (unsigned int)v << 8);
        return;
    }
    tsf_set_volume(tsfrenderer, v / 127.f);
}
...
#if SOUNDFONT
		/* The consumer runs from the sound ISR and this producer runs in
		 * trap context with interrupts enabled (SETIF=1), so the new index
		 * must be published in ONE store, already wrapped. The old two-step
		 * (widx++ then mask) had a window where the ISR could observe
		 * widx==4096 -- a value ridx's masked increment can never equal --
		 * and spin in VMPU_Process_Messages forever. */
		unsigned int w = vmpu.widx;
		vmpu.buffer[w] = value;
		vmpu.widx = (w + 1) & 0xfff;
#endif
	}
    ...
    for ( ;vmpu.ridx != vmpu.widx; vmpu.ridx++, vmpu.ridx &= 0xfff ) {
        if ( (vmpu.buffer[vmpu.ridx] & 0x80) && vmpu.buffer[vmpu.ridx] != 0xF7 ) {
            ...
            case 0xD: /* channel pressure +1 */
                synth_pressure(vmpu.channel, vmpu.buffer[vmpu.ridx]);
                break;
            ...
            case 0x8: /* note off +2 */
                if ( vmpu.didx == 0 )
                    ...
                    synth_note_off(vmpu.channel, vmpu.data[0]);
                    vmpu.didx = -1;
                }
                break;
            case 0x9: /* note on +2 */
                if ( vmpu.didx == 0)
                    ...
                    synth_note_on(vmpu.channel, vmpu.data[0], vmpu.buffer[vmpu.ridx]);
                    vmpu.didx = -1;
                }
                break;
            case 0xE: /* pitch bend +2 */
                if ( vmpu.didx == 0)
                    ...
                    synth_bend(vmpu.channel, vmpu.data[0] | (vmpu.buffer[vmpu.ridx] << 7));
                    vmpu.didx = -1;
                }
                break;
            case 0xC: /* program change +1 */
                synth_program(vmpu.channel, vmpu.buffer[vmpu.ridx]);
                break;
            case 0xB:
              ...
              */
                if ( vmpu.didx == 0)
                    ...
                    synth_control(vmpu.channel, vmpu.data[0], vmpu.buffer[vmpu.ridx]);
                    vmpu.didx = -1;
                }
                break;
            case 0xF: /* system +0 */
